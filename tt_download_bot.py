...
async def send_welcome(message: types.Message):
    ...
    await message.answer(languages[user_lang]["info"])
    await message.answer(languages[user_lang]["help"])
...
async def tt_download2(message: types.Message):
    ...
    try:
        ...
        if response.endswith(".mp3"):
            await message.reply_audio(open(response, 'rb'), caption='@ttdownloaderua_bot', title=link)
        # video
        else:
            logging.info(f"VIDEO: {response}")
            await message.reply_video(open(response, 'rb'), caption='@ttdownloaderua_bot',)
        os.remove(response)
